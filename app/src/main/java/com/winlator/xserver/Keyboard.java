...
public class Keyboard {
    ...
    public boolean onKeyEvent(KeyEvent event) {
        // ExternalController check skipped (not used in ExeLite)
        ...
        if (action == KeyEvent.ACTION_DOWN || action == KeyEvent.ACTION_UP) {
            ...
            if (chars != null && chars.length() == 1) {
                ...
                new android.os.Handler(android.os.Looper.getMainLooper()).postDelayed(() -> xServer.injectKeyRelease(xKeycode), 30);
            }
        }
        return true;
    }
